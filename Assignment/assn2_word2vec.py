...
def negSamplingLossAndGradient(
    ...
    z = np.dot(outsideVectors[outsideWordIdx], centerWordVec)
    loss -= np.log(sigmoid(z))
    gradCenterVec += (sigmoid(z) - 1) * outsideVectors[outsideWordIdx]
    gradOutsideVecs[outsideWordIdx] += (sigmoid(z) - 1) * centerWordVec

    for k in negSampleWordIndices:
        z = np.dot(outsideVectors[k], centerWordVec)
        loss -= np.log(sigmoid(-z))
        gradCenterVec += sigmoid(z) * outsideVectors[k]
        gradOutsideVecs[k] += sigmoid(z) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        current_loss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
